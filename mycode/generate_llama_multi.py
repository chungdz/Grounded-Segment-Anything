...
# class FakeArgs:
#     def __init__(self):
#         self.frame_info_path = 'llava_'
#         self.output_path = 'llama_'
#         self.rank = 0
# args = FakeArgs()
...
for findex in trange(378, flen, batch_size):
    ...
    for line in all_frames[findex: findex + batch_size]:
    
        line_dict = json.loads(line)
        video_ids.append(line_dict['video_id'])
        frame_ids.append(line_dict['frame_id']) 
        desc = line_dict['desc']
    ...
    input_ids = tokenizer(prompts, return_tensors="pt", padding=True).input_ids.to(0)
    generation_output = model.generate(input_ids=input_ids, max_length=2700, temperature=0.1, top_p=0.7, do_sample=True)
    res_list = tokenizer.batch_decode(generation_output, skip_special_tokens=True, clean_up_tokenization_spaces=False)
